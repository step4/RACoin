...
#if defined(NDEBUG)
# error "Racoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xeccc032753e65580fdda0662319d151c4180de17ac3a9a4578e37440f40c770f");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Racoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Racoin Signed Message:\n";
...
{
    // Racoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 210000); // Racoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Racoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Racoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Racoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Racoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
const char* pszTimestamp = "The start of RACoin, the start of something wonderful";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04b181100e4263236bcb114966a69a6ad2663b5c92c536178136bc073426439e31040d9f072c811b26a9bd519c1509cd711dd3e4a613cd289d297ea3c88922b5ab") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1517138528;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085420360;
        ...
        assert(block.hashMerkleRoot == uint256("0x643c72587b28e74f70b61f94b3d95d564a367dc2201127cd7f3818cb497ecb1c"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc1, 0xb1, 0xd3 }; // Racoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("RacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static RacoinMiner(CWallet *pwallet)
{
    printf("RacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("racoin-miner");
    ...
    try { loop {
        ...
        printf("Running RacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RacoinMiner, pwallet));
}
